...
std::vector<int> my::mergeSort(std::vector<int>& array) {
    ...
    for (size_t vectorIdx = 0; vectorIdx < array.size(); ++vectorIdx) {
        if (vectorIdx <= array.size() / 2 - 1) {
            leftArray.push_back(array[vectorIdx]);
        } else {
            rightArray.push_back(array[vectorIdx]);
        }
    }
